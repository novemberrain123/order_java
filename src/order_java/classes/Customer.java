...
public class Customer {
    ...
    // Called by member class to points static var (user) to a new member object
    // when new member signed up
    public static void pointsToNewMem() {
        user = new Member();
    }

    // Called by member class to points static var (user) to a new member object
    // when regular member login
    public static void pointsToRegMem(String name, int memberID, String password, double points) {
        user = new Member(name, memberID, password, points);
    }

    // Transfer customer order from a customer object to a new member object when
